...
class FactorGraph:
    ...
    def add_factors(self, ii, jj, remove=False):
        ...
        # place limit on number of factors if registered edges + added new edges > limit
        ...
        # update target and weight based on initial guess of pose
...
#         print("d_reshaped : \n", d_reshaped)
        ...
        for i in range(t0, t):
            if self.video.stereo:
                # dans le cas stereo on append les aretes identite i i  et on met la distance a inf
                es.append((i, i))
                # print("t : ", t, " t0 ", t0, " t1 ", t1, " i ", i)
                # print("(i-t0)*(t-t1) + (i-t1) : ", (i-t0)*(t-t1) + (i-t1))
                # set diagonal indices to inf
                d[(i-t0)*(t-t1) + (i-t1)] = np.inf

            # on ajoute les edges avec un radius rad + 1 autout de i
        ...
        for k in ix:
            ...
            # nms again to remove close edges patch losange dans la matrice d avec des axes 2 nms
            for di in range(-nms, nms+1):
                for dj in range(-nms, nms+1):
                    if abs(di) + abs(dj) <= max(min(abs(i-j)-2, nms), 0):
                        i1 = i + di
