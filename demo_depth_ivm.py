...
def image_stream_depth(datapath, use_depth=False, stride=1):
    ...
    image_list = sorted(glob.glob(os.path.join(datapath, 'left', '*.JPG')))[::stride]
    ...
    #intrinsics = [322.6092376708984, 322.6092376708984, 257.7363166809082, 186.6225147247314]
...
#    images += [cv2.remap(cv2.imread(images_right), map_r[0], map_r[1], interpolation=cv2.INTER_LINEAR)]
 ...
 #        cv2.imshow("rectified image",image)
        ...
        # cv2.imshow("depth before interpolation : ",depth)
        # cv2.waitKey(0)

        depth = torch.as_tensor(depth)
        depth = F.interpolate(depth[None,None], image_size).squeeze()

        # cv2.imshow("depth after interpolation : ",depth.numpy())
        # cv2.waitKey(0)
